...
class HostPIDPWMPositionController:
    def __init__(
        ...
        self.forward_lqr_csv_path = os.path.join(
            ...
            f"forward_lqr_irl_excited_{timestamp}.csv",
        )

        self.forward_lqr_mass_kg = 0.035

        # ====================================================
        # IRL PERSISTENT EXCITATION
        # ====================================================
        #
        # Used ONLY by UTA_lighthouse_irl_collect.py.
        #
        # Original UTA_lighthouse.py remains untouched.
        #
        # Smooth, small excitation is added only during
        # stable airborne hover.
        #
        self.irl_excitation_time = 0.0
        self.irl_excitation_announced = False

        # Base amplitudes.
        #
        # Two sinusoids are combined below, so actual peak
        # roll/pitch excitation is about 0.12 deg.
        #
        self.irl_eta_roll_scale_rad = np.deg2rad(0.08)
        self.irl_eta_pitch_scale_rad = np.deg2rad(0.08)

        # Actual peak vertical excitation is about
        # 0.15 m/s^2.
        self.irl_eta_az_scale = 0.10
        ...
        with SyncCrazyflie(self.uri, cf=self.cf) as scf:
            ...
            try:
                ...
                if not self.killed:
                    ...
                    # IRL TRAINING WINDOW
                    #
                    # Record ONLY this hover period.
                    #
                    # 0-7 s   : z_ref = 1.00 m
                    # 7-8 s   : z_ref = 0.90 m
                    # 8-15 s  : z_ref = 1.00 m
                    #
                    # Takeoff and landing are NOT recorded.
                    # ================================================

                    self.irl_record_hover_only = True
                    self.irl_excitation_time = 0.0
                    ...
                    try:
                        self._control_for(
                            ...
                            use_irl_perturbations=True,
                        )
                    finally:
                        self.irl_record_hover_only = False
    ...
    def _land_until_ground(self, duration_s: float):
        ...
        During landing:
            z > 0.040 m  -> keep landing
            z <= 0.040 m -> motors OFF immediately
    ...
    def _control_for(
        self,
        duration_s: float,
        follow_script_trajectory: bool = False,
        use_irl_perturbations: bool = False,
    ):
        print(
            "Control target "
        ...
        # Nominal hover point.
        base_hover_x = float(self.cf_setpoint.position.x)
        base_hover_y = float(self.cf_setpoint.position.y)
        base_hover_z = float(self.cf_setpoint.position.z)

        perturb1_announced = False
        recovery1_announced = False
        perturb2_announced = False
        recovery2_announced = False
        ...
        while time.monotonic() < end:
            ...
            # ================================================
            # IRL REFERENCE PERTURBATION SCHEDULE
            #
            # Nominal hover:
            #     (x,y,z) = (0.0, 0.0, 1.0)
            #
            ...
            #
            # 5-6 s:
            #     delta = (+0.10, 0.00, +0.10) m
            #
            # 6-13 s:
            #     nominal hover / recovery
            #
            # 13-14 s:
            #     delta = (0.00, -0.10, +0.10) m
            #
            # 14-21 s:
            #     nominal hover / recovery
            # ================================================

            if use_irl_perturbations:

                # Always start from nominal reference.
                self.cf_setpoint.position.x = base_hover_x
                self.cf_setpoint.position.y = base_hover_y
                self.cf_setpoint.position.z = base_hover_z

                # --------------------------------------------
                # First perturbation: t = 5 to 6 s
                # --------------------------------------------
                if 5.0 <= elapsed < 6.0:
                    ...
                    if not perturb1_announced:
                        print()
                        print(
                            "[IRL PERTURBATION 1] "
                            f"t={elapsed:.2f}s | "
                            "delta=(+0.10, +0.00, +0.10) m | "
                            f"ref=({base_hover_x + 0.10:+.2f}, "
                            f"{base_hover_y:+.2f}, "
                            f"{base_hover_z + 0.10:+.2f})"
                        )
                        print()

                        perturb1_announced = True

                elif (
                    elapsed >= 6.0
                    and perturb1_announced
                    and not recovery1_announced
                ):
                    ...
                    print(
                        "[IRL RECOVERY 1] "
                        f"t={elapsed:.2f}s | "
                        f"ref=({base_hover_x:+.2f}, "
                        f"{base_hover_y:+.2f}, "
                        f"{base_hover_z:+.2f})"
                    )
                    print()

                    recovery1_announced = True

                # --------------------------------------------
                # Second perturbation: t = 13 to 14 s
                # --------------------------------------------
                if 13.0 <= elapsed < 14.0:
                    ...
                    if not perturb2_announced:
                        ...
                        print(
                            ...
                            "delta=(+0.00, -0.10, +0.10) m | "
                            f"ref=({base_hover_x:+.2f}, "
                            f"{base_hover_y - 0.10:+.2f}, "
                            f"{base_hover_z + 0.10:+.2f})"
                        )
                        print()

                        perturb2_announced = True

                elif (
                    elapsed >= 14.0
                    and perturb2_announced
                    and not recovery2_announced
                ):
    ...
    def _control_step(self):
        ...
        if (self.outer_counter % self.outer_update_divider) == 0:
            ...
            # =================================================
            #
            # Expert target policy:
            #
            #       u* = -K* e
            #
            # During stable hover only, physically apply:
            #
            #       u_b = u* + eta
            #
            # where eta is a small smooth multi-sine signal.
            #
            # This gives the Q-function regression independent
            # action excitation while retaining the stabilizing
            # expert LQR.
            #
            # IMPORTANT:
            # lqr_out already contains the normal safe/clipped
            # expert command. We add a small eta and clip again
            # to the SAME safety limits.
            # =================================================

            excitation_active = (
                getattr(self, "irl_record_hover_only", False)
                and float(self.cf_state.position.z) >= 0.75
                and float(self.cf_setpoint.position.z) >= 0.80
                and abs(float(self.cf_setpoint.velocity.x)) < 0.02
                and abs(float(self.cf_setpoint.velocity.y)) < 0.02
                and abs(float(self.cf_setpoint.velocity.z)) < 0.02
            )

            if excitation_active:

                te = float(self.irl_excitation_time)

                eta_roll = (
                    self.irl_eta_roll_scale_rad
                    * (
                        np.sin(2.0 * np.pi * 0.53 * te)
                        + 0.5
                        * np.sin(
                            2.0 * np.pi * 1.17 * te
                            + 0.4
                ...
                self.irl_excitation_time += 0.01

                if not self.irl_excitation_announced:
                    print()
                    print(
                        "[IRL EXCITATION ACTIVE] "
                        "small roll/pitch/az multi-sine excitation"
                    )
                    print(
                        "roll/pitch peak ~= 0.12 deg, "
                        "az peak ~= 0.15 m/s^2"
                    )
                    print()

                    self.irl_excitation_announced = True
            ...
            # -------------------------------------------------
            # Behavior control actually sent to the vehicle.
            # -------------------------------------------------

            lqr_out.phi_des_rad = float(
                np.clip(
                    float(lqr_out.phi_des_rad)
                    + eta_roll,
                    -self.outer_lqr.max_angle_rad,
                    self.outer_lqr.max_angle_rad,
                )
            )

            lqr_out.theta_des_rad = float(
                np.clip(
                    float(lqr_out.theta_des_rad)
                    + eta_pitch,
                    -self.outer_lqr.max_angle_rad,
                    self.outer_lqr.max_angle_rad,
                )
            ...
            lqr_out.az_cmd_mps2 = float(
                np.clip(
                    float(lqr_out.az_cmd_mps2)
                    + eta_az,
                    -self.outer_lqr.max_az_mps2,
                    self.outer_lqr.max_az_mps2,
                )
            )
            ...
            # ------------------------------------------------
            if getattr(self, "irl_record_hover_only", False):
